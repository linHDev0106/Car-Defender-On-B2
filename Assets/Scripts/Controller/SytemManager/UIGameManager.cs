...
public class UIGameManager : Singleton<UIGameManager>
{
    ...
    private Transform transform_game_over;

    [SerializeField] private TextMeshPro text_game_over;
    [SerializeField] private TextMeshPro text_description_game_over;

    [SerializeField] private Transform   transform_new_wave;
    [SerializeField] private TextMeshPro text_new_wave;

    [SerializeField] private TextMeshPro text_description_tips_game;
    ...
    public IEnumerator<float> EnableNewWave (string value)
    {
        transform_new_wave.gameObject.SetActive (true);
        transform_game_over.gameObject.SetActive (false);

        text_new_wave.text = value;

        text_new_wave.transform.localScale = Vector.Vector3Zero;

        text_new_wave.transform.DOComplete ();
        text_new_wave.transform.DOScale (1f, Durations.DurationScale).SetEase (Ease.OutBack);
        ...
        transform_new_wave.gameObject.SetActive (false);

        yield return Timing.WaitForOneFrame;
    }

    public IEnumerator<float> EnableGameOver (string value, string description)
    {
        transform_game_over.gameObject.SetActive (true);
        transform_new_wave.gameObject.SetActive (false);

        text_game_over.text             = value;
        text_description_game_over.text = description;

        text_game_over.transform.localScale = Vector.Vector3Zero;

        text_game_over.transform.DOComplete ();
        text_game_over.transform.DOScale (1f, Durations.DurationScale).SetEase (Ease.OutBack);

        text_description_game_over.enabled = false;
        text_description_tips_game.enabled = false;

        text_description_tips_game.text = ApplicationLanguage.GetRandomTipsGame ();

        yield return Timing.WaitForSeconds (Durations.DurationScale);

        yield return Timing.WaitForSeconds (1f);

        text_description_game_over.enabled = true;
        text_description_tips_game.enabled = true;

        text_description_game_over.transform.localScale = Vector.Vector3Zero;
        text_description_tips_game.transform.localScale = Vector.Vector3Zero;

        text_description_tips_game.transform.DOComplete ();
        text_description_game_over.transform.DOComplete ();

        text_description_tips_game.transform.DOScale (1f, Durations.DurationScale).SetEase (Ease.OutBack);
        text_description_game_over.transform.DOScale (1f, Durations.DurationScale).SetEase (Ease.OutBack);
    ...
    public void OpenKittyShop ()
    {
        ...
        FindObjectOfType<AdsManager>().ShowInterstitialAd(0f);
    }
