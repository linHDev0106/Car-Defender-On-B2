...
public class UnlockManager : Singleton<UnlockManager>, IDialog
{
    ...
    public void UnlockItemLevel (int item_level)
    {
        ...
        var data = _ItemShopDetail.GetItem (item_level);

        _Level = item_level;

        _UiUnlockManager.Init (id, data.Speed, data.Earning, data.Damage, ApplicationLanguage.GetItemName (item_level), GameData.Instance.ItemDataGroup.GetDataItemWithLevel (item_level));
        _UiUnlockManager.SetShareFacebook (ApplicationManager.Instance.AppendFromUnit (_ShareData.GetRewardValue (_Level), _ShareData.GetRewardUnit (_Level)));

        this.PostMissionEvent (MissionEnums.MissionId.ReachItems, item_level);
