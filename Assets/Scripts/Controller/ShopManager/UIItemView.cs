...
public class UIItemView : MonoBehaviour
{
    ...
    private float damage_value_percents;

    private double last_price_notice;
    private int    last_price_unit_notice;

    private double damage_bullets;
    private int    damage_bullets_unit;

    private double damage_bullet_per_hit;
    private int    damage_bullet_per_hit_unit;

    private float damage_bullet_coefficient;
    ...
    public UIItemView InitParameters (CurrencyEnums.CurrencyId id, int _level)
    {
        ...
        damage_value_percents  = shop.Damage;

        var data_bullet = GameData.Instance.BulletsData.GetBullets (level);

        damage_bullet_per_hit      = data_bullet.Damage;
        damage_bullet_per_hit_unit = data_bullet.DamageUnit;
        damage_bullet_coefficient  = data_bullet.DamageCoefficient;

        price_diamonds = GameData.Instance.ItemDiamondPrices.GetPrice (level);

        Helper.FixNumber (ref price_data,ref price_unit_data);
    ...
    private void RefreshData ()
    {
        ...
        discount_upgrade_prices = UpgradeData.UpgradeValue * level_discount / 100f;
    ...
    public void RefreshUI (bool _is_unlock, bool _is_can_buy, bool _is_use_diamond)
    {
        ...
        else
        {
            ...
            damage_process.fillAmount  = damage_value_percents;
            ...
            label_item_name.text     = item_name;
            label_earning_items.text = string.Format (ApplicationLanguage.Text_description_earning_each_sec, ApplicationManager.Instance.AppendFromUnit (price_earning, price_earning_unit, 2));
            label_upgrade_level.text = string.Format (ApplicationLanguage.Text_description_lv, number_upgrade_times.ToString ());
            label_damage_items.text  = string.Format (ApplicationLanguage.Text_description_damage_each_hit, ApplicationManager.Instance.AppendFromUnit (damage_bullets, damage_bullets_unit, 2));
    ...
    {
        damage_bullets      = damage_bullet_per_hit * Math.Pow (damage_bullet_coefficient, number_upgrade_times);
        damage_bullets_unit = damage_bullet_per_hit_unit;

        Helper.FixUnit (ref damage_bullets, ref damage_bullets_unit);
    }
    ...
    private void OnUpgradeCompleted ()
    {
        ...
        this.PostActionEvent (ActionEnums.ActionID.RefreshUpgradeItems, level);
    ...
    public void InteractNextItems ()
    {
        ...
        SetId (GameData.Instance.ItemNodeIdleImage.GetShopIcon (level));
        RefreshUI (level <= PlayerData.LastLevelUnlocked, level <= PlayerData.LastLevelUnlocked, PlayerData.LastLevelUnlocked - 2 < level);
    }
